...
#pragma warning ( disable : 4131 4127 4244 4101 4996 4244 )
#endif
...
#define NEW_MIN_LOOKAHEAD (MAX_MATCH+MIN_MATCH)
...
{
  return ((sizeof(ush) + sizeof(uch)) * MT_DST_SCALE_FACTOR + sizeof(uch)) * s->lit_bufsize + /* pending_buf */
...
{
  s->d_buf = (ush*)s->pending_buf + s->lit_bufsize / sizeof(ush);
  s->l_buf = s->pending_buf + (sizeof(ush) * MT_DST_SCALE_FACTOR + sizeof(uch)) * s->lit_bufsize;
  s->window = s->pending_buf + ((sizeof(ush) + sizeof(uch)) * MT_DST_SCALE_FACTOR + sizeof(uch)) * s->lit_bufsize;
  { /* block */
...
{
  return (sizeof(ush) + sizeof(uch) * 2) * s->lit_bufsize + /* pending_buf */
  (s->w_size * 2) * sizeof(uch) + /* window */
...
{
  s->d_buf = (ush*)s->pending_buf + s->lit_bufsize / sizeof(ush);
  s->l_buf = s->pending_buf + (sizeof(ush) + sizeof(uch)) * s->lit_bufsize;
  s->window = s->pending_buf + (sizeof(ush) + sizeof(uch) * 2) * s->lit_bufsize;
    ...
    uInt  dictLength;
{
    ...
    if( ippStsNoErr != ippsDeflateDictionarySet_8u( dictionary, length,
                                                    ...
                                                    (Ipp32s*)s->prev,
                                                    s->window, s->w_size, s->level ) ) return Z_STREAM_ERROR;
...
#else
#ifdef IPP_DEFAULT_COMPRESSION
    if (level == Z_DEFAULT_COMPRESSION) level = 5;
#else
    if (level == Z_DEFAULT_COMPRESSION) level = 6;
#endif
#endif
    ...
    int flush;
{
    ...
    if (strm->avail_in != 0 || s->lookahead != 0 ||
        ...
        else                bstate = deflate_common(s, flush);
                              ...
                              mt_state* mt_s, deflate_state *s, IppLZ77Flush ipp_flush )
{
  ...
  do {
    if( src_offset > 0 ) {
      ...
      if( src_offset >= w_size ) { /* the dictionary is in a source block fully */
        if( ippStsNoErr != ippsDeflateUpdateHash_8u( src - w_size, match_start - w_size, w_size + MIN_MATCH - 1,
                                                     head, hash_size, prev, w_size, s->level ) ) break;
        window = src;
      } /* if */
      else {
        window = window_buff;
        if( match_start < w_size ) { /* a stream begining */
          uInt src_part_len = match_start - s->strstart;
          zmemcpy( window + w_size - match_start, s->window, s->strstart );
          zmemcpy( window + w_size - src_part_len, src - src_part_len, src_part_len + MAX_MATCH );
          if( ippStsNoErr != ippsDeflateUpdateHash_8u( window + w_size - match_start, 0, match_start + MIN_MATCH - 1,
                                                       head, hash_size, prev, w_size, s->level ) ) break;
        } /* if */
        else { /* a block begining */
          uInt dict_part_len = w_size - src_offset;
          zmemcpy( window, s->window + s->strstart - dict_part_len, dict_part_len );
          zmemcpy( window + dict_part_len, src - src_offset, src_offset + MAX_MATCH );
          if( ippStsNoErr != ippsDeflateUpdateHash_8u( window, match_start - w_size, w_size + MIN_MATCH - 1,
                                                       head, hash_size, prev, w_size, s->level ) ) break;
        } /* else */
        window += w_size;
      } /* else */
    ...
    { /*block */
      ...
      if( ippStsNoErr != ippsDeflateLZ77_8u( (const Ipp8u**)&src, &mt_s->src_len, &match_start,
                                             ...
                                             lit_dst, dist_dst, &mt_s->dst_len, s->level, ipp_flush ) ) break;
...
#pragma omp parallel for num_threads(chunks) reduction(|: ret)
  ...
  } while( (*src_len >= NEW_MIN_LOOKAHEAD) && (dst_len > 0) );
} /* lz77_encode_call */
...
local void lz77_encode_call( uch** src, uInt* src_len, deflate_state *s, int flush )
{
  ...
  IppStatus status = ippsDeflateLZ77_8u( (const Ipp8u**)src, src_len, &s->match_start,
                                         ...
                                         s->level, ( flush == Z_NO_FLUSH ) ? IppLZ77NoFlush : IppLZ77FullFlush );
...
local block_state deflate_common(deflate_state* s, int flush)
{
  ...
  if( (s->match_start + s->lookahead + s->strm->avail_in + wsize) >= (uInt)0x7fffffff ) {
    ...
    if( s->level >= 4 ) normalize( (int)norm_level, (int*)s->prev, (int)wsize );
  } /* if */
  while( (s->lookahead > 0) || (s->strm->avail_in > 0) ) {
    if( (s->strm->avail_in < wsize) || ((s->window_size - s->strstart - s->lookahead) < NEW_MIN_LOOKAHEAD) ) {
      ...
      if( (s->lookahead < NEW_MIN_LOOKAHEAD) && (flush == Z_NO_FLUSH) ) return need_more;
      if( s->lookahead == 0 ) break;
      lz77_encode_window( s, flush );
    } /* if */
    else {
      if( s->lookahead > 0 ) {
        UPDATE_WINDOW(NEW_MIN_LOOKAHEAD);
        zmemcpy( s->window + s->strstart + s->lookahead, s->strm->next_in, NEW_MIN_LOOKAHEAD );
        s->lookahead += NEW_MIN_LOOKAHEAD;
        lz77_encode_window( s, flush );
        if( NEW_MIN_LOOKAHEAD > s->lookahead ) {
          read_buf( s->strm, s->window + s->strstart + s->lookahead - NEW_MIN_LOOKAHEAD,
                    NEW_MIN_LOOKAHEAD - s->lookahead);
          s->lookahead = 0;
        } /* if */
        else {
          lz77_encode_window( s, flush );
      ...
      if( s->last_lit < s->lit_bufsize ) {
        UPDATE_WINDOW(NEW_MIN_LOOKAHEAD);
        zmemcpy( s->window + s->strstart, s->strm->next_in, NEW_MIN_LOOKAHEAD );
